...
def load_user_data(sheet_url):
    try:
        ...
        df = pd.read_csv(csv_export_url,usecols=["username","name","password","access state"]).fillna("")
        return df
...
# Step 2: Build user_credentials and user_state_map
...
for _, row in user_df.iterrows():
    ...
    state = row["access state"].strip()

    # Create credential entry only once
    if username not in user_credentials["usernames"]:
...
#         "gaurav":{"name": "Gaurav Kumar", "password": "gaurav123"},
#         "ravi":{"name": "Ravi", "password": "ravi123"},
#         "ravi1":{"name": "Bihar User", "password": "bihar123"},
#     }
# }

# # --- Username to allowed state mapping ---
# user_state_map = {
#     "admin": ["ALL"],
#     "gaurav": ["ALL"],
#     "ravi": ["Assam", "Bihar"],
...
hashed_passwords = stauth.Hasher(
    [user_credentials["usernames"][u]["password"] for u in user_credentials["usernames"]]
).generate()

for i, u in enumerate(user_credentials["usernames"]):
    user_credentials["usernames"][u]["password"] = hashed_passwords[i]

# Initialize login
authenticator = stauth.Authenticate(
    credentials=user_credentials,
    cookie_name="comm_dashboard_login",
    key="some_random_key_123",  # can be anything
...
if auth_status is False:
    ...
    st.warning("🛡️ Please enter your Username & Password")
    st.stop()
else:
...
with st.sidebar:
    with st.expander("🎛️ Apply Filters", expanded=True):
        ...
        if "ALL" in allowed_states:
            ...
            # selected_states = allowed_states
            # st.markdown(f"📍 **States Allowed:** {', '.join(allowed_states)} (Access Restricted)")
            selected_states = st.multiselect("📍 State", state_options, default=st.session_state.state_filter, key="state_filter")
