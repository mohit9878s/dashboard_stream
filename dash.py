...
with st.sidebar:
    ...
    with st.expander("🎛️ Apply Filters", expanded=True):
        ...
        state = st.multiselect(
            ...
            default=st.session_state.state_filter,
            key="state_filter")

        vendor = st.multiselect(
            "🏷️ Vendor",
            sorted(df["Vendor Name"].unique()),
            default=st.session_state.vendor_filter,
            key="vendor_filter")

        cohort = st.multiselect(
            "🎯 Cohort",
            sorted(df["Cohort"].unique()),
...
if compact_view:
    format_func = format_full_decimal if compact_style == "Full Format (e.g. 2,57,08,228 Cr)" else format_compact_decimal
else:
    format_func = format_indian_number
